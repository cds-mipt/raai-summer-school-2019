...
class CarRacingHackaton(gym.Env, EzPickle):
    metadata = {
    ...
    def step(self, action):
        ...
        if CarRacingHackaton.training_epoch:
            if done:
                with open("training_positions.csv", 'a') as fin:
                    fin.write(','.join(list(map(str, [CarRacingHackaton.training_epoch,
                                        self.car.hull.angle,
                                        self.car.hull.position.x,
                                        self.car.hull.position.y]))))
                    fin.write('\n')
...
if __name__=="__main__":
    ...
    if args.using_start_file:
        env = CarRacingHackaton(agent=args.no_agent, write=args.write, data_path=args.dir,
                        start_file=args.using_start_file,
                        training_epoch=1)
    else:
        env = CarRacingHackaton(agent=args.no_agent, num_bots=args.bots_number,
